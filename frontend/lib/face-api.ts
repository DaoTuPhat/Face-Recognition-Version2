...
export async function verifyUserFace(
  ...
  try {
    ...
    // Sử dụng API utility để gửi form data
    const result = await api.postForm("users/attendance", formData);
    ...
    let message = "Đã xảy ra lỗi khi xử lý ảnh. Vui lòng thử lại.";
    if (error instanceof Error) {
      message = error.message;
      if (message.includes("Attendance has already been recorded for today")) {
        message = "Bạn đã điểm danh hôm nay rồi.";
      } else if (message.includes("Attendance record not found for today.")) {
        message = "Không tìm thấy bản ghi điểm danh cho hôm nay. Vui lòng liên hệ quản trị viên.";
      }
    } else {
      message = typeof error === 'string' ? error : "Đã xảy ra lỗi không xác định. Vui lòng thử lại.";
    }
    return {
      success: false,
      message,
